...
type SetTitle = { newTitle: string }

export const TitleContractInteractions: FC = () => {
  const sorobanContext = useSorobanReact()


  const [, setFetchIsLoading] = useState<boolean>(false)
  const [updateIsLoading, setUpdateIsLoading] = useState<boolean>(false)
  const { register, handleSubmit } = useForm<SetTitle>()
  ...
  // Fetch Greeting
  const fetchCurrentTitle = useCallback(async () => {
  ...
  useEffect(() => {void fetchCurrentTitle()}, [updateFrontend,fetchCurrentTitle])


  const { activeChain, server, address } = sorobanContext

  const updateTitle = async ({ newTitle }: SetTitle ) => {
    if (!address) {
      ...
      if (!currentChain) {
        ...
        try {
          const result = await contract?.invoke({
            method: 'set_title',
            args: [nativeToScVal(newTitle, {type: "string"})],
            signAndSend: true
          })
  ...
  if(!contract){
    return (
      <div tw="flex grow flex-col space-y-4 max-w-[20rem]">
        <h2 tw="text-center font-mono text-gray-400">Title Smart Contract</h2>
  ...
  return (
    <>
      <div tw="flex grow flex-col space-y-4 max-w-[20rem]">
        ...
        <Card variant="outline" p={4} bgColor="whiteAlpha.100">
          <FormControl>
            <FormLabel>Fetched Title</FormLabel>
            <Input
          ...
          <form onSubmit={handleSubmit(updateTitle)}>
            <Stack direction="row" spacing={2} align="end">
              <FormControl>
                ...
                <Input disabled={updateIsLoading} {...register('newTitle')} />
              </FormControl>
              <Button
                type="submit"
                mt={4}
