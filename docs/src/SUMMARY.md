# Summary
# Where to start
- [A script and a boilerplate](hello.md)
# Create Soroban Script
- [Usage](./create-soroban-dapp/overview.md)
- [Manual cloning](create-soroban-dapp/manual_cloning.md)
- [Troubleshooting](create-soroban-dapp/troubleshooting.md)
- [Set your Secrets in .env](create-soroban-dapp/set_your_secrets.md)

# Soroban Dapp Boilerplate
- [Overview](./soroban-dapp/overview.md)
- [Getting started](soroban-dapp/getting_started.md)
    - [Verify installation](soroban-dapp/instructions/verify_installation.md)
    - [Customize app data](soroban-dapp/instructions/1.md)
- [Manage contracts](soroban-dapp/contracts/manage_contracts.md)
    - [Contract folder organization](./soroban-dapp/contracts/contract_folder_organization.md)
    - [Create and build your own contracts](soroban-dapp/contracts/create_and_build_contracts.md)
    - [Deploy your contracts using the provided script](soroban-dapp/contracts/deploy_contracts.md)
    - [Retrieve the contract object in the frontend](soroban-dapp/contracts/frontend_contract_object.md)
- [Fly with your own wings](./soroban-dapp/fly_alone.md)

# Other Resources
- [SOROCHAT dapp tutorial](./other_resources/sorochat.md)
